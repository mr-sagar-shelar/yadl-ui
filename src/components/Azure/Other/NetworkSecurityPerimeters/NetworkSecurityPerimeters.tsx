import type { SVGProps } from "react";
import { memo } from "react";
const NetworkSecurityPerimeters = (props: SVGProps<SVGSVGElement>) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="1em"
    height="1em"
    viewBox="0 0 18 18"
    {...props}
  >
    <defs>
      <linearGradient
        id="prefix__a"
        x1={3.397}
        x2={3.397}
        y1={2.087}
        y2={10.659}
        gradientUnits="userSpaceOnUse"
      >
        <stop offset={0.167} stopColor="#50e6ff" />
        <stop offset={1} stopColor="#32bedd" />
      </linearGradient>
      <linearGradient
        id="prefix__b"
        x1={-1258.471}
        x2={-1258.471}
        y1={827.868}
        y2={834.423}
        gradientTransform="rotate(180 -626.176 422.808)"
        gradientUnits="userSpaceOnUse"
      >
        <stop offset={0} stopColor="#32bedd" />
        <stop offset={0.826} stopColor="#50e6ff" />
      </linearGradient>
      <linearGradient
        id="prefix__c"
        x1={11.505}
        x2={11.505}
        y1={0.126}
        y2={6.41}
        gradientUnits="userSpaceOnUse"
      >
        <stop offset={0.167} stopColor="#50e6ff" />
        <stop offset={1} stopColor="#32bedd" />
      </linearGradient>
    </defs>
    <path
      fill="#0078d4"
      d="M15.712 4.536v3.1a.136.136 0 0 1-.205.118 2.96 2.96 0 0 0-1.485-.4h-.078a3 3 0 0 0-.293.015.136.136 0 0 1-.147-.137V3.091a.382.382 0 0 1 .62-.3l.02.019L15.6 4.266l.025.028a.4.4 0 0 1 .052.083.4.4 0 0 1 .035.159M4.668 2.162H7.4a.273.273 0 0 1 .193.467l-1.7 1.7a.14.14 0 0 1-.1.04H3.222a.381.381 0 0 1-.269-.65L4.4 2.274a.38.38 0 0 1 .268-.112"
    />
    <path
      fill="url(#prefix__a)"
      d="M4.51 2.162v5.016a.154.154 0 0 0 .154.154h1.717a.15.15 0 0 1 .118.049.153.153 0 0 1-.006.217L3.545 10.53a.153.153 0 0 1-.209 0L.361 7.6a.164.164 0 1 1 .112-.31h1.705a.14.14 0 0 0 .122-.154V4.514a.35.35 0 0 1 .1-.245Z"
    />
    <path
      fill="#0078d4"
      d="M2.294 13.206v-2.732a.274.274 0 0 1 .468-.193l1.7 1.7a.13.13 0 0 1 .04.1v2.574a.381.381 0 0 1-.65.269l-1.446-1.448a.39.39 0 0 1-.112-.27"
    />
    <path
      fill="url(#prefix__b)"
      d="M2.294 13.364h4.254a.154.154 0 0 0 .152-.154v-1.716a.15.15 0 0 1 .049-.119.153.153 0 0 1 .217.006L9.9 14.329a.153.153 0 0 1 0 .209l-2.932 2.976a.164.164 0 0 1-.308-.114v-1.7a.14.14 0 0 0-.153-.124H4.646a.35.35 0 0 1-.245-.1Z"
    />
    <path
      fill="url(#prefix__c)"
      d="M15.71 4.379h-5.047a.14.14 0 0 0-.075.041.15.15 0 0 0-.048.109v1.72a.15.15 0 0 1-.049.119l-.023.019a.154.154 0 0 1-.192-.026L7.342 3.414a.154.154 0 0 1 0-.21L10.275.229s.1-.128.209-.1c.087.023.1.119.1.21v1.7a.2.2 0 0 0 .015.05.13.13 0 0 0 .136.074h2.624a.3.3 0 0 1 .1.015l.033.011a.4.4 0 0 1 .111.075l2.021 2.022Z"
    />
    <path
      fill="#ffbd02"
      d="M17.619 12.58h-.006a.2.2 0 0 0-.039-.031.2.2 0 0 0-.052-.029.2.2 0 0 0-.035-.014.1.1 0 0 0-.033-.01h-.612v-1.71a3.22 3.22 0 0 0-.82-2.161 2.725 2.725 0 0 0-4.079 0 3.16 3.16 0 0 0-.821 2.161v1.694h-.468l-.038.006a.4.4 0 0 0-.126.045l-.027.017-.035.027-.019.018-.019.021a1 1 0 0 0-.038.052.3.3 0 0 0-.029.06l-.01.031-.008.032c0 .012-.005.023-.006.035v4.408a.4.4 0 0 0 .034.149l.014.03a.4.4 0 0 0 .107.124h.007a.4.4 0 0 0 .221.073h6.691a.37.37 0 0 0 .231-.082.38.38 0 0 0 .15-.3v-4.365a.38.38 0 0 0-.135-.281m-5.174-1.822a1.74 1.74 0 0 1 .477-1.184 1.37 1.37 0 0 1 1.93-.147 1.3 1.3 0 0 1 .148.147 2 2 0 0 1 .179.238 1.7 1.7 0 0 1 .24.447 1.7 1.7 0 0 1 .091.5v1.722h-3.065Z"
    />
    <path
      fill="#ffe452"
      d="M17.747 12.861v4.373a.38.38 0 0 1-.15.3l-3.565-2.468h-.007l-3.588-2.483a.37.37 0 0 1 .176-.087l.038-.006h6.8a.1.1 0 0 1 .033.01.2.2 0 0 1 .035.014.2.2 0 0 1 .052.029.2.2 0 0 1 .039.031h.006a.38.38 0 0 1 .131.287"
      opacity={0.4}
    />
    <path
      fill="#ffe452"
      d="M17.613 12.576h-.007l-3.58 2.48h-.007l-3.571 2.473h-.006a.4.4 0 0 1-.115-.128l-.013-.03a.4.4 0 0 1-.021-.121v-4.437q-.002-.018.006-.035l.008-.032.01-.031a.3.3 0 0 1 .029-.06 1 1 0 0 1 .038-.052l.016-.003.019-.018.035-.027.027-.017a.3.3 0 0 1 .112-.045l.038-.006h6.813a.1.1 0 0 1 .033.01.2.2 0 0 1 .035.014.2.2 0 0 1 .052.029.2.2 0 0 1 .049.036"
      opacity={0.4}
    />
  </svg>
);
const Memo = memo(NetworkSecurityPerimeters);
export default Memo;
