import type { SVGProps } from "react";
import { memo } from "react";
const DefenderCMLocalManager = (props: SVGProps<SVGSVGElement>) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="1em"
    height="1em"
    fill="none"
    viewBox="0 0 18 18"
    {...props}
  >
    <path
      fill="url(#prefix__a)"
      d="m15.733 3.502-1.238-.032h-.148l-2.19-.327-1.408-.622L9.861 0 7.819.021l-.096.285L7.025 2.5l-1.407.601-2.391-1.055L1.809 3.47l.148.285 1.058 2.036-.56 1.403L0 8.112v2.057l.286.095 2.2.675.603 1.403-1.058 2.394 1.471 1.424.264-.147 2.042-1.055.656.801 1.693 2.226H10.2l.095-.284.73-1.288 1.164-.58 2.307-.137 1.439-1.466v-.137l.285-1.055.836-2.5zm-6.73 10.095a4.63 4.63 0 0 1-2.575-.775 4.597 4.597 0 0 1-.692-7.106 4.63 4.63 0 0 1 5.053-.976 4.6 4.6 0 0 1 2.068 1.716 4.6 4.6 0 0 1 .76 2.574 4.6 4.6 0 0 1-1.363 3.231 4.62 4.62 0 0 1-3.25 1.336"
    />
    <path
      fill="url(#prefix__b)"
      d="M9.003 12.745c.047.088.14.16.258.2l4.356 1.708a1.615 1.615 0 0 0 1.089.024l3.065-1.319a.3.3 0 0 0 .123-.073.2.2 0 0 0 .062-.106.1.1 0 0 0 0-.04.3.3 0 0 0-.096-.142.5.5 0 0 0-.173-.095l-4.348-1.707a1.613 1.613 0 0 0-1.089-.024l-3.074 1.317a.28.28 0 0 0-.153.105.16.16 0 0 0-.02.152"
    />
    <path
      fill="#003067"
      d="m15.022 17.543-.01.011-.251.107-.011.006a1.6 1.6 0 0 1-1.09-.024l-4.355-1.707c-.213-.087-.304-.205-.26-.309l-.047-2.883c.048.089.14.16.259.201l4.356 1.708a1.615 1.615 0 0 0 1.09.024l.011-.005 3.053-1.315a.3.3 0 0 0 .121-.073.2.2 0 0 0 .062-.106l.048 2.95a.1.1 0 0 1 0 .04.22.22 0 0 1-.063.106.34.34 0 0 1-.122.074l-.112.047z"
    />
    <path
      fill="#32B0E7"
      fillRule="evenodd"
      d="m11.153 14.336-1.44-.604v.232l1.44.604zm0 .592-1.434-.602v.233l1.434.601zm.24.333v-.233l1.749.734v.232zm-.24.262-1.434-.6v.232l1.434.6zm.24.333v-.232l1.749.733v.233zm0-1.187v-.233l1.742.73v.233z"
      clipRule="evenodd"
    />
    <path
      fill="#50E6FF"
      d="M16.807 14.77c-.116-.049-.116-.206 0-.351.115-.146.31-.226.42-.173.108.053.113.205 0 .35-.114.144-.31.222-.42.174"
    />
    <path
      fill="url(#prefix__c)"
      d="M9.003 8.358a.45.45 0 0 0 .258.2l4.349 1.706a1.612 1.612 0 0 0 1.087.024l3.06-1.317a.3.3 0 0 0 .122-.074.2.2 0 0 0 .062-.105.1.1 0 0 0 0-.04.3.3 0 0 0-.095-.142.5.5 0 0 0-.173-.094L13.332 6.81a1.61 1.61 0 0 0-1.087-.024l-3.07 1.315a.28.28 0 0 0-.152.105.16.16 0 0 0-.02.151"
    />
    <path
      fill="#003067"
      d="m15.012 13.149-.01.01-.25.108-.012.005a1.6 1.6 0 0 1-1.087-.023l-4.349-1.704c-.212-.087-.303-.205-.26-.309l-.046-2.878a.45.45 0 0 0 .258.2l4.35 1.706a1.612 1.612 0 0 0 1.087.024l.012-.006 3.047-1.312a.3.3 0 0 0 .122-.074.2.2 0 0 0 .061-.105l.049 2.945a.1.1 0 0 1 0 .04.2.2 0 0 1-.064.106.3.3 0 0 1-.121.074l-.112.047z"
    />
    <path
      fill="#32B0E7"
      fillRule="evenodd"
      d="m11.15 9.947-1.438-.603v.232l1.437.603zm0 .59-1.432-.6v.232l1.431.6zm.239.333v-.232l1.746.732v.232zm-.24.262-1.431-.6v.232l1.431.6zm.24.333v-.232l1.746.732v.232zm0-1.186v-.232l1.74.73v.232z"
      clipRule="evenodd"
    />
    <path
      fill="#50E6FF"
      d="M16.794 10.38c-.115-.049-.115-.205 0-.35.115-.146.31-.226.42-.173.108.053.113.205 0 .35-.114.143-.31.221-.42.173"
    />
    <path
      fill="url(#prefix__d)"
      d="M9.003 3.707a.45.45 0 0 0 .258.2l4.349 1.705a1.612 1.612 0 0 0 1.087.024l3.06-1.317a.3.3 0 0 0 .122-.073.2.2 0 0 0 .062-.106.1.1 0 0 0 0-.04.3.3 0 0 0-.095-.141.5.5 0 0 0-.173-.095L13.332 2.16a1.61 1.61 0 0 0-1.087-.024l-3.07 1.315a.28.28 0 0 0-.152.104.16.16 0 0 0-.02.152"
    />
    <path
      fill="#003067"
      d="m15.012 8.497-.01.011-.25.107-.012.006a1.6 1.6 0 0 1-1.087-.024L9.304 6.893c-.212-.087-.303-.205-.26-.308l-.046-2.878a.45.45 0 0 0 .258.2l4.35 1.706a1.612 1.612 0 0 0 1.087.024l.012-.006 3.047-1.313a.3.3 0 0 0 .122-.073.2.2 0 0 0 .061-.106l.049 2.946a.1.1 0 0 1 0 .04.2.2 0 0 1-.064.106.3.3 0 0 1-.121.073l-.112.048z"
    />
    <path
      fill="#32B0E7"
      fillRule="evenodd"
      d="m11.15 5.295-1.438-.602v.232l1.437.602zm0 .591-1.432-.6v.232l1.431.6zm.239.333v-.232l1.746.732v.232zm-.24.262-1.431-.6v.232l1.431.6zm.24.333V6.58l1.746.733v.232zm0-1.186v-.232l1.74.73v.231z"
      clipRule="evenodd"
    />
    <path
      fill="#50E6FF"
      d="M16.794 5.728c-.115-.048-.115-.205 0-.35s.31-.225.42-.172c.108.053.113.205 0 .349-.114.144-.31.222-.42.173"
    />
    <defs>
      <linearGradient
        id="prefix__a"
        x1={8.527}
        x2={8.527}
        y1={0}
        y2={17.985}
        gradientUnits="userSpaceOnUse"
      >
        <stop stopColor="#7A7A7A" />
        <stop offset={1} stopColor="#454545" />
      </linearGradient>
      <linearGradient
        id="prefix__b"
        x1={13.475}
        x2={13.475}
        y1={11.089}
        y2={14.759}
        gradientUnits="userSpaceOnUse"
      >
        <stop stopColor="#5EA0EF" />
        <stop offset={0.18} stopColor="#559CEC" />
        <stop offset={0.47} stopColor="#3C91E5" />
        <stop offset={0.84} stopColor="#1380DA" />
        <stop offset={1} stopColor="#0078D4" />
      </linearGradient>
      <linearGradient
        id="prefix__c"
        x1={13.468}
        x2={13.468}
        y1={6.706}
        y2={10.369}
        gradientUnits="userSpaceOnUse"
      >
        <stop stopColor="#5EA0EF" />
        <stop offset={0.18} stopColor="#559CEC" />
        <stop offset={0.47} stopColor="#3C91E5" />
        <stop offset={0.84} stopColor="#1380DA" />
        <stop offset={1} stopColor="#0078D4" />
      </linearGradient>
      <linearGradient
        id="prefix__d"
        x1={13.468}
        x2={13.468}
        y1={2.054}
        y2={5.718}
        gradientUnits="userSpaceOnUse"
      >
        <stop stopColor="#5EA0EF" />
        <stop offset={0.18} stopColor="#559CEC" />
        <stop offset={0.47} stopColor="#3C91E5" />
        <stop offset={0.84} stopColor="#1380DA" />
        <stop offset={1} stopColor="#0078D4" />
      </linearGradient>
    </defs>
  </svg>
);
const Memo = memo(DefenderCMLocalManager);
export default Memo;
