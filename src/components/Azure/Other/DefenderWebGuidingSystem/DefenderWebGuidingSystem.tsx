import type { SVGProps } from "react";
import { memo } from "react";
const DefenderWebGuidingSystem = (props: SVGProps<SVGSVGElement>) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="1em"
    height="1em"
    fill="none"
    viewBox="0 0 18 18"
    {...props}
  >
    <g clipPath="url(#prefix__a)">
      <path
        fill="url(#prefix__b)"
        d="M4.722 12.08c0 .119.162.215.363.215h7.93a.5.5 0 0 0 .112-.014.3.3 0 0 0 .095-.039.2.2 0 0 0 .064-.06.13.13 0 0 0 .023-.07V8.836q-.001-.077-.085-.13a.4.4 0 0 0-.207-.054H5.082c-.2 0-.363.096-.363.214z"
      />
      <path
        fill="#fff"
        d="M5.494 9.325a.145.145 0 1 0 0-.289.145.145 0 0 0 0 .29M12.65 9.325a.145.145 0 1 0 0-.289.145.145 0 0 0 0 .29M12.65 12a.145.145 0 1 0 0-.289.145.145 0 0 0 0 .29M5.494 12a.145.145 0 1 0 0-.289.145.145 0 0 0 0 .29"
        opacity={0.6}
      />
      <path
        fill="#fff"
        d="M6.242 9.593H6.11c-.062 0-.113.03-.113.07v1.623c0 .038.05.07.113.07h.132c.063 0 .113-.032.113-.07V9.662c0-.038-.05-.07-.113-.07M6.824 9.593H6.69c-.062 0-.113.03-.113.07v1.623c0 .038.05.07.113.07h.133c.062 0 .113-.032.113-.07V9.662c0-.038-.051-.07-.113-.07M7.404 9.593h-.132c-.062 0-.113.03-.113.07v1.623c0 .038.05.07.113.07h.132c.063 0 .113-.032.113-.07V9.662c0-.038-.05-.07-.113-.07"
        opacity={0.7}
      />
      <path
        fill="#004C87"
        d="M2.335 9.905a.42.42 0 0 1-.422.42.42.42 0 0 1-.422-.42.42.42 0 0 1 .422-.42.42.42 0 0 1 .422.42M16.114 10.324a.42.42 0 0 0 .421-.42.42.42 0 0 0-.421-.418.42.42 0 0 0-.422.419c0 .231.189.42.422.42"
      />
      <path
        fill="#004C87"
        fillRule="evenodd"
        d="M4.117 8.347c0-1.008.823-1.826 1.837-1.826h6.118c1.015 0 1.837.818 1.837 1.826V9.7a2.354 2.354 0 0 0 2.362 2.347c.335-.013.59-.106.896-.208l-.157-.497-.74.183A1.83 1.83 0 0 1 14.435 9.7V8.347A2.354 2.354 0 0 0 12.072 6H5.954a2.354 2.354 0 0 0-2.361 2.347V9.7a1.83 1.83 0 0 1-1.837 1.826L.94 11.31l-.145.542c.301.138.527.167.96.195A2.354 2.354 0 0 0 4.118 9.7zM2.335 9.905a.42.42 0 0 1-.422.42.42.42 0 0 1-.422-.42.42.42 0 0 1 .422-.42.42.42 0 0 1 .422.42m.525 0c0 .52-.424.94-.947.94a.944.944 0 0 1-.947-.94c0-.52.424-.94.947-.94s.947.42.947.94m13.254.42a.42.42 0 0 0 .421-.42.42.42 0 0 0-.421-.42.42.42 0 0 0-.422.42c0 .231.189.42.422.42m0 .52a.944.944 0 0 0 .946-.94c0-.52-.424-.94-.946-.94a.944.944 0 0 0-.947.94c0 .52.424.94.947.94"
        clipRule="evenodd"
      />
      <path
        fill="url(#prefix__c)"
        d="m3.296 8.785-.097-.147a1.78 1.78 0 0 0-1.37-.69h-.043a1.795 1.795 0 0 0-1.66 1.125l-.072.23q-.027.11-.038.22A2 2 0 0 0 0 9.681c0 .018.006.17.012.227s.006.098.014.146q.03.147.08.286c.009.023.013.047.022.07q.075.18.185.341l.004.005a1.79 1.79 0 0 0 3.26-1.02c-.001-.338-.099-.668-.281-.952m-1.918.951a.404.404 0 1 1 .808-.001.404.404 0 0 1-.808.001m1.9.508a1.63 1.63 0 0 0-.887-.504v-.004a.6.6 0 0 0-.158-.4c.206-.269.503-.454.835-.521a1.53 1.53 0 0 1 .21 1.429m-1.632-2.08c-.137.319-.17.672-.095 1.01a.6.6 0 0 0-.349.429 1.4 1.4 0 0 1-.914-.366 1.58 1.58 0 0 1 1.36-1.073zM.59 10.768c.343-.075.652-.258.884-.52.092.06.2.095.312.098a.6.6 0 0 0 .177-.036c.165.298.215.646.14.978q-.157.031-.316.032a1.58 1.58 0 0 1-1.195-.55z"
      />
      <path
        fill="url(#prefix__d)"
        d="M2.391 9.74c.342.069.653.246.887.504a1.53 1.53 0 0 0-.21-1.43 1.4 1.4 0 0 0-.835.522c.1.11.157.252.158.4z"
      />
      <path
        fill="url(#prefix__e)"
        d="M.59 10.767c.342-.075.651-.258.883-.52.092.06.2.095.312.098a.6.6 0 0 0 .177-.036c.165.298.215.646.14.978q-.157.031-.316.032a1.58 1.58 0 0 1-1.195-.55z"
      />
      <path
        fill="url(#prefix__f)"
        d="M1.646 8.165c-.137.318-.17.67-.095 1.009a.6.6 0 0 0-.349.429 1.4 1.4 0 0 1-.914-.366 1.58 1.58 0 0 1 1.36-1.073z"
      />
      <path
        fill="url(#prefix__g)"
        d="m17.72 8.808-.097-.146a1.78 1.78 0 0 0-1.37-.69h-.043a1.8 1.8 0 0 0-1.661 1.124l-.072.23q-.026.11-.037.22a1 1 0 0 0-.016.16c0 .018.006.17.011.227.006.057.006.098.015.146q.029.146.08.285.01.035.021.07.075.181.185.341l.005.006a1.789 1.789 0 0 0 3.26-1.021c-.002-.338-.099-.668-.282-.952m-1.918.952a.403.403 0 0 1 .483-.398.405.405 0 1 1-.483.398m1.9.507a1.63 1.63 0 0 0-.887-.503V9.76a.6.6 0 0 0-.159-.4c.207-.27.504-.454.836-.522a1.53 1.53 0 0 1 .21 1.43M16.07 8.188c-.138.318-.17.671-.095 1.01a.6.6 0 0 0-.349.428 1.4 1.4 0 0 1-.914-.366 1.58 1.58 0 0 1 1.36-1.072zm-1.057 2.603c.343-.075.652-.258.883-.521.093.061.201.095.312.098a.6.6 0 0 0 .178-.035c.165.297.214.645.14.977q-.157.031-.316.033a1.58 1.58 0 0 1-1.196-.551z"
      />
      <path
        fill="url(#prefix__h)"
        d="M16.815 9.764c.342.068.653.245.887.503a1.53 1.53 0 0 0-.21-1.429 1.4 1.4 0 0 0-.835.521c.1.11.157.252.158.4z"
      />
      <path
        fill="url(#prefix__i)"
        d="M15.013 10.79c.343-.074.652-.257.883-.52.093.061.201.095.312.098a.6.6 0 0 0 .178-.035c.165.297.214.645.14.977q-.157.031-.316.033a1.58 1.58 0 0 1-1.196-.551z"
      />
      <path
        fill="url(#prefix__j)"
        d="M16.07 8.188c-.138.318-.17.671-.095 1.01a.6.6 0 0 0-.349.428 1.4 1.4 0 0 1-.914-.366 1.58 1.58 0 0 1 1.36-1.072z"
      />
    </g>
    <defs>
      <linearGradient
        id="prefix__b"
        x1={8.585}
        x2={8.585}
        y1={8.294}
        y2={12.595}
        gradientUnits="userSpaceOnUse"
      >
        <stop stopColor="#5EA0EF" />
        <stop offset={1} stopColor="#0078D4" />
      </linearGradient>
      <linearGradient
        id="prefix__c"
        x1={1.788}
        x2={1.788}
        y1={11.525}
        y2={7.948}
        gradientUnits="userSpaceOnUse"
      >
        <stop stopColor="#0078D4" />
        <stop offset={0.156} stopColor="#1380DA" />
        <stop offset={0.528} stopColor="#3C91E5" />
        <stop offset={0.822} stopColor="#559CEC" />
        <stop offset={1} stopColor="#5EA0EF" />
      </linearGradient>
      <linearGradient
        id="prefix__d"
        x1={1.788}
        x2={1.788}
        y1={11.525}
        y2={7.948}
        gradientUnits="userSpaceOnUse"
      >
        <stop stopColor="#0078D4" />
        <stop offset={0.156} stopColor="#1380DA" />
        <stop offset={0.528} stopColor="#3C91E5" />
        <stop offset={0.822} stopColor="#559CEC" />
        <stop offset={1} stopColor="#5EA0EF" />
      </linearGradient>
      <linearGradient
        id="prefix__e"
        x1={1.788}
        x2={1.788}
        y1={11.525}
        y2={7.948}
        gradientUnits="userSpaceOnUse"
      >
        <stop stopColor="#0078D4" />
        <stop offset={0.156} stopColor="#1380DA" />
        <stop offset={0.528} stopColor="#3C91E5" />
        <stop offset={0.822} stopColor="#559CEC" />
        <stop offset={1} stopColor="#5EA0EF" />
      </linearGradient>
      <linearGradient
        id="prefix__f"
        x1={1.788}
        x2={1.788}
        y1={11.525}
        y2={7.948}
        gradientUnits="userSpaceOnUse"
      >
        <stop stopColor="#0078D4" />
        <stop offset={0.156} stopColor="#1380DA" />
        <stop offset={0.528} stopColor="#3C91E5" />
        <stop offset={0.822} stopColor="#559CEC" />
        <stop offset={1} stopColor="#5EA0EF" />
      </linearGradient>
      <linearGradient
        id="prefix__g"
        x1={16.212}
        x2={16.212}
        y1={11.548}
        y2={7.972}
        gradientUnits="userSpaceOnUse"
      >
        <stop stopColor="#0078D4" />
        <stop offset={0.156} stopColor="#1380DA" />
        <stop offset={0.528} stopColor="#3C91E5" />
        <stop offset={0.822} stopColor="#559CEC" />
        <stop offset={1} stopColor="#5EA0EF" />
      </linearGradient>
      <linearGradient
        id="prefix__h"
        x1={16.212}
        x2={16.212}
        y1={11.548}
        y2={7.972}
        gradientUnits="userSpaceOnUse"
      >
        <stop stopColor="#0078D4" />
        <stop offset={0.156} stopColor="#1380DA" />
        <stop offset={0.528} stopColor="#3C91E5" />
        <stop offset={0.822} stopColor="#559CEC" />
        <stop offset={1} stopColor="#5EA0EF" />
      </linearGradient>
      <linearGradient
        id="prefix__i"
        x1={16.212}
        x2={16.212}
        y1={11.548}
        y2={7.972}
        gradientUnits="userSpaceOnUse"
      >
        <stop stopColor="#0078D4" />
        <stop offset={0.156} stopColor="#1380DA" />
        <stop offset={0.528} stopColor="#3C91E5" />
        <stop offset={0.822} stopColor="#559CEC" />
        <stop offset={1} stopColor="#5EA0EF" />
      </linearGradient>
      <linearGradient
        id="prefix__j"
        x1={16.212}
        x2={16.212}
        y1={11.548}
        y2={7.972}
        gradientUnits="userSpaceOnUse"
      >
        <stop stopColor="#0078D4" />
        <stop offset={0.156} stopColor="#1380DA" />
        <stop offset={0.528} stopColor="#3C91E5" />
        <stop offset={0.822} stopColor="#559CEC" />
        <stop offset={1} stopColor="#5EA0EF" />
      </linearGradient>
      <clipPath id="prefix__a">
        <path fill="#fff" d="M0 0h18v18H0z" />
      </clipPath>
    </defs>
  </svg>
);
const Memo = memo(DefenderWebGuidingSystem);
export default Memo;
